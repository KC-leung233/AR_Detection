...
% the detection scheme by using different original data or threshold.
clear; clc;
...
load ERA5IVT_98th_2020.mat % Load the threshold of ARs
data_input_path = ''; % The path of your original data
data_save_path = ''; % The path of your output data
for yr = 1 : length(Total_yr)
    IVT_Total = load(strcat(data_input_path, string(Total_yr(yr)), '.mat'),'IVT'); % Load the original yearly IVT matrix
    ...
    for mo  = 1 : length(Total_mo)
        vIVT = vIVT_Total(:, :, Part_Detect_Series == Total_mo(mo)); % Archive the IVT for a specific month
        vIVT1 = vIVT;
        ...
        % Save the monthly original IVT and the potential ARs
        save(strcat(data_save_path, 'IVT_', ...
...
for yr = 1 : length(Total_yr)
    ...
    for mo  = 1 : length(Total_mo)
        ...
        load(strcat(data_save_path, 'IVT_', ...
            datestr(datetime(Total_yr(yr), Total_mo(mo), 1), 'yyyymm'), '.mat'), 'IVT', 'IVT1');
        if Rerun == 1
